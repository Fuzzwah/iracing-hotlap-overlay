...
def update_speed_rpm():
    ...
    if not rpm is None:
        ...
        rpm = max(0, min(rpm, 1))
        vert_line = cfg['rpm_speed']['vertical_line']
        blocks = cfg['rpm_speed']['blocks']
        rpm = vert_line + (
    ...
    if speed is None:
        ...
        if len(state.speed_calc_data) > 1:
            ...
            if diff_pct > 0 and diff_time > 0:
                ...
                if speed > 110:
                    speed = 0
    if cfg['speed_units'] == 'mps':                 # meters per second
        speed = '{:2.0f} m/s'.format(speed)
    elif cfg['speed_units'] == 'mph':               # miles per hour
        speed = '{:2.0f} mph'.format(speed * 2.236936)
    else:                                           # else, you get kilometers per hour
        speed = '{:2.0f} km/h'.format(speed * 3.6)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-V', '--version', action='version', version='iRacing Hotlap Overlay %s' % VERSION, help='show version and exit')
    ...
    logging.info('iRacing Hotlap Overlay %s' % VERSION)
    ...
    try:
        ...
        logging.error("Unable to successfully read config file: %s" % args.configfile)
        sys.exit(1)
    ...
    try:
        if args.test or args.dump:
            ...
            app = QApplication([])
            controlsWindow = ControlsWindow(ir, state, cfg)
            timer = QtCore.QTimer();
